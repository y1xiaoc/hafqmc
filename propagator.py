...
class Propagator(nn.Module):
    ...
    def setup(self):
        ...
        self.ts_v = self.param("ts_v", fix_init, _ts_v, _dtype) if _pd["tsteps"] else _ts_v
        self.ts_h = self.param("ts_h", fix_init, _ts_h, _dtype) if _pd["tsteps"] else _ts_h
        ...
        self.wfn_packed = self.param("wfn", fix_init, _wfn_packed, _dtype) if _pd["wfn"] else _wfn_packed
        self.nbasis = self.wfn_packed.shape[0]
        # core energy, should be useless
        self.enuc = self.param("enuc", fix_init, self.init_enuc, _t_real) if _pd["enuc"] else self.init_enuc
        ...
        if self.auxfieldnet is None:
            AuxFieldCls = AuxField
            network_args = {}
        else:
            AuxFieldCls = AuxFieldNet
            network_args = self.auxfieldnet if isinstance(self.auxfieldnet, dict) else {"hidden_sizes": self.auxfieldnet}
        AuxFieldCls = nn.vmap(AuxFieldCls,
                              in_axes=0,
                              out_axes=0,
                              axis_size=self.nts_v,
                              variable_axes={'params': 0 if (_vd["vhs"] or _vd["vnet"]) else None},
                              split_rngs={'params': _vd["vhs"] or _vd["vnet"]})
        self.vhs_ops = AuxFieldCls(_vhs,
