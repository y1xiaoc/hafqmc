...
from . import symmetric_matrices
...
def init_training_metrics(num_statistics):
  ...
  if not num_statistics:
    return TrainingMetrics(jnp.array(0, _fdtype))
  else:
    return TrainingMetrics(jnp.zeros([num_statistics], _fdtype))
...
def init_training_metrics_shapes(num_statistics):
  ...
  # num_statistics=0.
  if not num_statistics:
    return TrainingMetrics([[], _fdtype])
  else:
    return TrainingMetrics([[num_statistics], _fdtype])
...
def matrix_inverse_pth_root(
  ...
  if matrix_size == 1:
    ...
    error = jnp.max(jnp.abs(mat_m - identity)).astype(_fdtype)
...
def _add_error_into_local_stats(local_stats, errors, inverse_failure_threshold):
  ...
  for local_stat in local_stats:
    if local_stat.sizes:
      ...
      per_stat_error = jnp.array(0, _fdtype)
    if local_stat.sizes:
...
def distributed_shampoo(
  ...
  def quantized_dtype_for_second_moment_statistics_buffers():
    return jnp.int16 if best_effort_memory_usage_reduction and batch_axis_name else _fdtype
  ...
  def _quantize_diagonal_statistics(diagonal_statistics):
    return QuantizedValue.from_float_value(diagonal_statistics, _fdtype)
  ...
  def sharded_init_fn(params):
    ...
    for param in params_flat:
      ...
      if not _skip_preconditioning(param):
        ...
        shapes = preconditioner.shapes_for_preconditioners()
        statistics = [
            matrix_epsilon * jnp.eye(max_size, dtype=_fdtype)
            for s in shapes
        ]
        preconditioners = [jnp.eye(max_size, dtype=_fdtype) for s in shapes]
  ...
  def sharded_init_shape_and_dtype_fn(params):
    ...
    global_stats = GlobalShardedParameterStats([statistics_shape, _fdtype],
                                               [statistics_shape, _fdtype],
  ...
  def sharded_update_fn(grads, state, params):
    ...
    new_stats_flat = jax.tree_util.tree_map(
    ...
    if preconditioning_compute_steps == 1:
      ...
      errors_init = jnp.ones([n], _fdtype) * inverse_failure_threshold
  ...
  def init_fn(params):
    ...
    def _init(param):
      ...
      if not _skip_preconditioning(param):
        shapes = preconditioner.shapes_for_preconditioners()
        statistics = [
            matrix_epsilon * jnp.eye(s[0], dtype=_fdtype) for s in shapes
        ]
        preconditioners = [jnp.eye(s[0], dtype=_fdtype) for s in shapes]
  ...
  def _pmap_compute_preconditioners(states, step, statistics,
    ...
    for num_statistics, state in zip(num_statistics_per_state, states):
      if num_statistics == 0:
        ...
        errors_for_states.append(jnp.array(0, _fdtype))
      else:
  ...
  def _pmap_quantized_compute_preconditioners(states, step, statistics,
    ...
    to_pad = -num_statistics % num_devices
    padded_eye = jnp.eye(max_size, dtype=_fdtype)
